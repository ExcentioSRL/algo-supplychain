...
import example_boxes as eb
import example_boxes_member as bm

record_codec = ABIType.from_string(str(eb.MembershipRecord().type_spec()))
...
def main() -> None:
    ...
    app_client = beaker.client.ApplicationClient(
        beaker.localnet.get_algod_client(), eb.app, signer=acct.signer,app_id=53
    )
    print("Creating app")
    app_client.create()
    ...
    ptxn = PaymentTxn(
        ...
        sp,
        app_client.app_addr,
        eb.app.state.minimum_balance.value,
    )
    ...
    app_member_client = beaker.client.ApplicationClient(
        beaker.localnet.get_algod_client(), bm.app, signer=app_client.signer,app_id=52
    )
    ...
    app_member_client.call(
        bm.get_affirmation, boxes=[(app_client.app_id, "affirmations")]
    )
